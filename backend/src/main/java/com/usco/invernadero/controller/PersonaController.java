...
public class PersonaController {
    ...
    @GetMapping
    public ResponseEntity<List<Persona>> getAll(Authentication auth) {
        return ResponseEntity.ok(service.findAll(resolverUsuario(auth)));
    }

    @GetMapping("/{id}")
    public ResponseEntity<Persona> getById(@PathVariable Long id, Authentication auth) {
        return ResponseEntity.ok(service.findById(id, resolverUsuario(auth)));
    }

    @PostMapping
    public ResponseEntity<Map<String, Object>> create(
            @Valid @RequestBody Persona entity, Authentication auth) {
        entity.setUsuario(resolverUsuario(auth));
        Persona saved = service.save(entity);
    ...
    public ResponseEntity<Map<String, Object>> update(
            ...
            @Valid @RequestBody Persona entity, Authentication auth) {
        Persona updated = service.update(id, entity, resolverUsuario(auth));
