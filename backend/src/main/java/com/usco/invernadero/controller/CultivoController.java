...
public class CultivoController {
    ...
    @GetMapping
    public ResponseEntity<List<Cultivo>> getAll(Authentication auth) {
        return ResponseEntity.ok(service.findAll(resolverUsuario(auth)));
    }

    @GetMapping("/{id}")
    public ResponseEntity<Cultivo> getById(@PathVariable Long id, Authentication auth) {
        return ResponseEntity.ok(service.findById(id, resolverUsuario(auth)));
    }

    @PostMapping
    public ResponseEntity<Map<String, Object>> create(
            @Valid @RequestBody Cultivo entity, Authentication auth) {
        entity.setUsuario(resolverUsuario(auth));
        Cultivo saved = service.save(entity);
    ...
    public ResponseEntity<Map<String, Object>> update(
            ...
            @Valid @RequestBody Cultivo entity, Authentication auth) {
        Cultivo updated = service.update(id, entity, resolverUsuario(auth));
