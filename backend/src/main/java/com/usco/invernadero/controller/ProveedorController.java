...
public class ProveedorController {
    ...
    @GetMapping
    public ResponseEntity<List<Proveedor>> getAll(Authentication auth) {
        return ResponseEntity.ok(service.findAll(resolverUsuario(auth)));
    }

    @GetMapping("/{id}")
    public ResponseEntity<Proveedor> getById(@PathVariable Long id, Authentication auth) {
        return ResponseEntity.ok(service.findById(id, resolverUsuario(auth)));
    }

    @PostMapping
    public ResponseEntity<Map<String, Object>> create(
            @Valid @RequestBody Proveedor entity, Authentication auth) {
        entity.setUsuario(resolverUsuario(auth));
        Proveedor saved = service.save(entity);
    ...
    public ResponseEntity<Map<String, Object>> update(
            ...
            @Valid @RequestBody Proveedor entity, Authentication auth) {
        Proveedor updated = service.update(id, entity, resolverUsuario(auth));
